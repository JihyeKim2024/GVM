...
random_device rd;
splitmix gen(rd);
unsigned int N = 100000, n_size = N - 1, t_ensemble = 1;
...
int main()
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        double gam = 3.9 + 0.1 * i; //exponent of power law P(s)
        denom[i] = 0;
        for (int j = N; j > 1; --j)
        {
            denom[i] += pow(j, 1 - gam);
        ...
        {
            int cs = ss + 2;
            double p_cs = pow(cs, 1 - gam) / denom[i];
    ...
    for (int hyper_en = 0; hyper_en < 100000000; ++hyper_en)
    {
        ...
        {
            sprintf(fname, "sim_alpha3.9_4.1_en%d.txt", t_ensemble);
            fp = fopen(fname, "w");
            for (int i = 0; i < 3; ++i)
            {
                fprintf(fp, "%.9f\t%.9f\t%.9f\n", 3.9 + 0.1 * i, aver_ep[i], aver_et[i]);
            }
            fclose(fp);
        }
        ++t_ensemble;
